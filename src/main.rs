...
fn log_level_from_config(config: &Config) -> LogLevel {
    if config.quiet {
        LogLevel::Quiet
...
fn main() {
    ...
    let level = log_level_from_config(&config);
...
fn run_git_config(key: &str, value: &str) -> Result<()> {
...
fn install_pre_commit_hook(hook_path: &Path) -> Result<()> {
...
fn install_gitattributes(attributes_path: &Path) -> Result<()> {
...
fn run_install_git_hooks() -> Result<()> {
    ...
    install_pre_commit_hook(&hook_path)?;

    run_git_config("merge.electrolysis.name", "Electrolysis pbxproj merge driver")?;
    run_git_config("merge.electrolysis.driver", "electrolysis merge-driver %O %A %B")?;
    eprintln!("{} configured merge driver", "✓".green().bold());

    install_gitattributes(&attributes_path)?;
...
fn config_from_cli(cli: &Cli, base: Option<Config>) -> Config {
    ...
    macro_rules! set_if_some {
        ($cli_field:ident => $cfg_field:ident) => {
            if let Some(v) = cli.$cli_field { config.$cfg_field = v; }
        };
        ($field:ident) => {
            if let Some(v) = cli.$field { config.$field = v; }
        };
    }
    set_if_some!(quiet);
    set_if_some!(verbose);
    set_if_some!(sanitize_only);
    set_if_some!(unique => unique_only);
    set_if_some!(sort => sort_only);
...
fn resolve_path_if_relative(path: &Path) -> Result<PathBuf> {
    if path.is_absolute() {
        Ok(path.to_path_buf())
    } else {
...
fn run_process(config: Config, cli: Cli) -> Result<()> {
    ...
    if config.watch {
        ...
        return run_watch(config, resolve_path_if_relative(&paths[0])?);
    }
    ...
    for path in &paths {
        let path = resolve_path_if_relative(path)?;
...
fn pick_xcodeprojs(found: Vec<PathBuf>) -> Result<Vec<PathBuf>> {
    ...
    let choice: usize = if trimmed.is_empty() {
        ...
        match trimmed.parse() {
            Ok(n) => n,
            Err(_) => bail!("invalid selection: '{}' — please enter a number", trimmed),
        }
    };
    ...
    if choice == all_idx {
        ...
        bail!("invalid selection: {} — valid range is 1–{}", choice, all_idx)
    }
}
